#!/bin/bash
#
# AISAC Installer — Wazuh Agent + AISAC Agent (Linux)
#
# Single script that:
#   1. Fetches tenant config via install-config edge function
#   2. Installs Wazuh Agent pointing to the centralized Wazuh Manager
#   3. Installs AISAC Agent with heartbeat, collector, and optionally SOAR
#
# Usage:
...
# =============================================================================
# Step 1: Fetch tenant config from install-config edge function
# =============================================================================
...
fetch_config() {
    ...
    log_info "Fetching config from: ${config_url}"
    ...
    if [ "$http_code" != "200" ]; then
        log_error "install-config returned HTTP ${http_code}: ${response}"
        exit 1
    fi
...
# =============================================================================
# Step 3: Install AISAC Agent
# =============================================================================

# ── Config loading & URL validation ──
    ...
    local payload=$(cat <<EOF
{
    ...
    "version": "1.0.5",
    "capabilities": ${capabilities}${cs_fields}
}
EOF
)
...
registration:
  enabled: true
  url: "$(echo "$HEARTBEAT_URL" | sed -E 's|/functions/v1/.*||')/functions/v1/agent-register"
  api_key: "${API_KEY}"
  auth_token: "${AUTH_TOKEN}"
  asset_id: "${ASSET_ID}"
...
main() {
    local ARG_API_KEY="" ARG_AUTH_TOKEN="" ARG_MANAGER_IP="" ARG_CONFIG_URL="$DEFAULT_CONFIG_URL"

    # Parse arguments
    while [ $# -gt 0 ]; do
        case "$1" in
            -k) ARG_API_KEY="$2"; shift 2 ;;
            -t) ARG_AUTH_TOKEN="$2"; shift 2 ;;
            -m) ARG_MANAGER_IP="$2"; shift 2 ;;
            -u) ARG_CONFIG_URL="$2"; shift 2 ;;
    ...
    if [ -z "$ARG_AUTH_TOKEN" ]; then
        log_error "Auth token is required (-t)"
        missing=true
    fi
    if [ -z "$ARG_MANAGER_IP" ]; then
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Step 1: Fetch config from install-config edge function
    # ══════════════════════════════════════════════════════════════════════════
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}  Step 1/3: Fetching tenant configuration                     ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo ""

    fetch_config "$ARG_API_KEY" "$ARG_CONFIG_URL" "$ARG_AUTH_TOKEN"

    # Parse config for Wazuh installation
    local manager_port agent_group asset_name asset_id wazuh_agent_name
    ...
    if [ -z "$agent_group" ]; then
        log_error "Missing wazuh.agent_group in config response"
        exit 1
    fi

    [ -z "$asset_name" ] && asset_name=$(hostname)

    # Sanitize asset name: Wazuh rejects spaces and special chars in agent names
    asset_name=$(echo "$asset_name" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]/-/g; s/-\+/-/g; s/^-//; s/-$//')
    ...
    if [ -n "$asset_id" ]; then
        wazuh_agent_name="${asset_name}_${asset_id}"
    else
        log_warning "No asset_id in config response, using asset_name only"
    ...
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}  Step 2/3: Installing Wazuh Agent                            ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo ""

    detect_distro
    install_wazuh_agent "$ARG_MANAGER_IP" "${manager_port:-1514}" "$agent_group" "$wazuh_agent_name"

    # Extract Wazuh agent ID after installation
    ...
    # Inject wazuh metadata into the config JSON for AISAC agent registration
    if command -v jq &>/dev/null; then
        local tmp_json
        tmp_json=$(jq \
            --arg name "$wazuh_agent_name" \
...
with open('$REGISTER_OUTPUT') as f:
    data = json.load(f)
data.setdefault('wazuh', {})['agent_name'] = '$wazuh_agent_name'
...
"
    ...
    log_info "  Wazuh Agent Name: ${wazuh_agent_name}"
    log_info "  Wazuh Agent ID:   ${wazuh_agent_id:-unknown}"
    log_success "Wazuh Agent installed → Manager: ${ARG_MANAGER_IP} | Group: ${agent_group}"
    ...
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}  Step 3/3: Installing AISAC Agent                            ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    if [ "$SOAR_ENABLED" = "true" ]; then
        ...
        echo -e "  ${CYAN}Certs:${NC}        ${CONFIG_DIR}/certs/"
    fi
